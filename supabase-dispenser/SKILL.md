...
4. Return `DATABASE_URL` for Postgres projects, plus `DATABASE_POOL_URL` when PgBouncer is enabled. For Supabase projects, also return `SUPABASE_URL`, anon key, service role key, pooler URL, and Studio URL when present.
